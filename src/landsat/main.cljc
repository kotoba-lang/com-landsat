(ns landsat.main
  "Kotodama WASM entrypoint for the Landsat clean-room actor (L4) — Clojure port.
  ...
  facts (`landsat.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "landsat")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"landsat.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "landsat-compat" :tier tier :entities entities} 200])
...
(defn start! [] :landsat-compat/ready)
